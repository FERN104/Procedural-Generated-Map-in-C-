...
public partial class Player : Entity
{
    private void PlayerMovement()
    {
        ...
        float
            dx = (targetpos.X -
                  globalPhysics.position.X); // Redefine with target position to keep going to clicked location
        ...
        {
            globalPhysics.velocity.X =
                (dx / distance) *
                globalPhysics
                    .speed; // this uses the vector normalisation formula to create a pure direction (Distance is 1).
            globalPhysics.velocity.Y =
                (dy / distance) * globalPhysics.speed; // Formula: Normalised = Vector / Magnitude
    ...
    private Dictionary<KeyboardKey, Func<Spell.Spell>> SpellCaster = new()
    {
